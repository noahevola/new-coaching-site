// src/pages/OSProgrammeNewsletter.tsx
...
function InlineCTA({ text = 'Install The OS', className = '' }: InlineCTAProps) {
  ...
  const handleClick = () => {
    try {
      sessionStorage.setItem('applicationSource', 'os-programme-newsletter');
    } catch (e) {
      // ignore errors
    }

    navigate('/apply?source=os-programme-newsletter');
...
function OSProgrammeNewsletter() {
  const navigate = useNavigate();
...
export default OSProgrammeNewsletter;
