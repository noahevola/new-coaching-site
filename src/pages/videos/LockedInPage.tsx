// src/pages/videos/LockedInPage.tsx
...
function InlineCTA({ text = 'Install The OS', className = '' }: InlineCTAProps) {
  ...
  const handleClick = () => {
    try {
      sessionStorage.setItem('applicationSource', 'locked-in');
    } catch (e) {
      // ignore errors
    }

    navigate('/apply?source=locked-in');
